...
from mingpt.model_tit import GPT_TIT, GPTConfig_TIT
...
if args.model_type == 'reward_conditioned' or args.model_type == 'naive':
    mconf = GPTConfig(train_dataset.vocab_size, train_dataset.block_size,
                      n_layer=6, n_head=8, n_embd=128, model_type=args.model_type, max_timestep=max(timesteps))
    model = GPT(mconf)
elif args.model_type == 'reward_conditioned_tit':
    mconf = GPTConfig_TIT(train_dataset.vocab_size, train_dataset.block_size,
                      n_layer=6, n_head=8, n_embd=128, model_type=args.model_type, max_timestep=max(timesteps))
    model = GPT_TIT(mconf)
